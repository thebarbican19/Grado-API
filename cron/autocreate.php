...
$trending_query = mysqli_query("SELECT like_timestamp, like_content, COUNT(*) AS likes_count, GROUP_CONCAT(content_tags SEPARATOR ',') AS like_tags, GROUP_CONCAT(like_user SEPARATOR ',') AS like_users FROM likes LEFT JOIN content ON likes.like_content LIKE content.content_key WHERE like_timestamp > '$trending_expiry' AND content_hidden = 0 GROUP BY like_content HAVING likes_count > 0 AND like_users NOT LIKE '$authuser_key' ORDER BY likes_count DESC");
$trending_likes = 0;
$trending_tags = array();
...
function channel_image($tag, $location, $type) {
	if ($type == "trending") {
		$post_query = mysqli_query("SELECT `content_images` FROM  `content` WHERE `content_tags` LIKE '%$tags%' AND `content_images` NOT LIKE '' ORDER BY `content_timestamp` DESC LIMIT 0 ,1");
	...
	elseif ($type == "local") {
		...
		$flickr_search = file_get_contents($flickr_search_url);
		...
		$flickr_image = file_get_contents($flickr_image_url);
...
function channel_create($title, $description, $tags, $type, $location, $owner) {
	...
	if ($channel_exists == 0) {
		...
		else echo 'failed creating channel ' . $title . " - " . mysql_error() . "\n";
