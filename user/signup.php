...
if ($passed_method == 'POST') {
	...
	elseif (empty($passed_username)) {
		...
		$email_query = mysqli_query("SELECT * FROM `users` WHERE `user_email` LIKE '$passed_email' LIMIT 0, 1");
		$email_exists = mysql_num_rows($email_query);
		$email_output = mysql_fetch_assoc($email_query);
		...
		if ($email_status == "active") {
			...
			$username_query = mysqli_query("SELECT * FROM `users` WHERE `user_name` LIKE '$passed_username' LIMIT 0, 1");
			...
			if ($username_exists) {
				...
				if ($user_create) {
					...
					$bearer_create = mysqli_query("INSERT INTO `access` (`access_id`, `access_created`, `access_expiry`, `access_token`, `access_user`, `access_app`, `access_scope`) VALUES (NULL, CURRENT_TIMESTAMP, '$bearer_expiry', '$bearer_token', '$user_key', '$session_application', '$bearer_scope');");
					if ($bearer_create) {
		...
		$existing_query = mysqli_query("SELECT * FROM `users` WHERE (`user_email` LIKE '$passed_email' OR `user_name` LIKE '$passed_username') LIMIT 0, 1");
	...
	if (in_array("admin", $authentication_scope)) {
		if (empty($session_bearer)) {
			...
			$json_output[] = array('status' => $json_status, 'error_code' => '348');
			echo json_encode($json_output);
			exit;
			
		}
		else {
			$user_remove = mysqli_query("UPDATE `users` SET `user_status` = 'deactivated' WHERE `user_key` LIKE '$authuser_key';");
			if ($user_remove) {
