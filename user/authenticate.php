	...
	elseif (empty($passed_password)) {
		...
		if ($user_exists == 1) {
			...
			if (password_verify($passed_password ,$user_password)) {
				...
				if ($bearer_create)	 {
					$user_update = mysqli_query("UPDATE `users` SET `user_status` = 'active' WHERE `user_key` LIKE '$user_key';");
					$user_output = array("key" => $user_key, "username" => $user_name, "actualname" => $user_fullname, "email" => $user_email, "type" => $user_type, "slack" => $user_slack, "avatar" => $user_avatar, "city" => $user_city, "country" => $user_country, "langauge" => $user_language, "lastactive" => $user_lastactive, "signup" => $user_signup, "subscriptions" => $subcsription_data, "auth" => $bearer_output, "stats" => $user_stats);	
				...
				$json_output[] = array('status' => $json_status, 'error_code' => '351');
				echo json_encode($json_output);
				exit;
...
elseif ($passed_method == 'DELETE') {
	$destroy_token = mysqli_query("DELETE FROM `access` WHERE `access_token` LIKE '$session_bearer';");
	if ($destroy_token) {
