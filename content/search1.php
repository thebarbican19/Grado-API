...
if ($passed_method == 'GET') {
	if (empty($passed_search)) {
		$trending_sql = "SELECT `trending_keyword`, `trending_user`, `trending_latitude`, `trending_longitude`, COUNT(*) AS trending_count FROM trending LEFT JOIN content on subscriptions.subscription_channel LIKE content.  GROUP BY trending_keyword ORDER BY COUNT(*) DESC, trending_timestamp DESC LIMIT $passed_pagenation, $passed_limit";
		$trending_query = mysqli_query($trending_sql);
		
		/*
		$expiry_query = mysqli_query("SELECT * FROM `trending` ORDER BY `trending_id` ASC LIMIT 0, 12");
		...
		*/
		...
		$json_output[] = array('status' => $json_status, 'error_code' => 200, 'output' => $search_output, 'sql' => $trending_sql);
