...
if (!in_array($session_page, $session_exclude) || $session_method == "DELETE" || isset($session_bearer)) {
	if (empty($session_bearer)) {	
		...
		if ($authentication_exists == 0) {
			...
			$authuser_query = mysqli_query($database_grado_connect ,"SELECT * FROM `users` WHERE `user_key` LIKE '$authentication_user' AND `user_status` LIKE 'active'");
			...
			if ($authuser_exists == 0) {
				...
				if (isset($session_latlng)) {
					...
					$location_data = mysql_fetch_assoc($location_query);
				...
				else $authuser_update = mysqli_query($database_grado_connect, "UPDATE `users` SET `user_lastactive` = CURRENT_TIMESTAMP WHERE `user_key` LIKE '$authuser_key';");
